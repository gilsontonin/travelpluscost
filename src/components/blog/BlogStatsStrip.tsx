...
export default function BlogStatsStrip({ city, hotelCount, avgRating }: { city: string; hotelCount: number; avgRating: number | null }) {
  ...
  return (
    <div className="mt-3 flex flex-wrap items-stretch gap-2">
      {stats.map((s) => (
        <div key={s.label} className="flex min-w-0 flex-1 items-baseline gap-1.5 rounded-xl border border-black/[0.08] bg-zinc-50 px-3 py-2">
          <span className="text-lg font-bold tracking-tight text-accent">{s.n}</span>
          <span className="truncate text-xs leading-tight text-black/55">{s.label}</span>
        </div>
      ))}
    </div>
  );
