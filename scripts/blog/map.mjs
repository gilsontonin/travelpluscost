...
const todo = entries.filter((e) => !e.doneBy).sort((a, b) => a.tier - b.tier || b.totalVol - a.totalVol);
const doneList = entries.filter((e) => e.doneBy).sort((a, b) => b.totalVol - a.totalVol);

const today = new Date().toISOString().slice(0, 10);
const L = [];
L.push(`# Keyword map — ${city}`);
L.push(`_priority queue · quick wins first · source: ${file} · generated ${today}_`);
...
L.push(`\n**Tiers by quick-win difficulty** (easiest decent-volume entry): T1 = KD ≤18 · T2 = KD 19–30 · T3 = KD 31+ (high-value, slower on a young domain). Volume/KD are Semrush ballpark — broad is fine.`);

const fmt = (e) => {
  const lines = [];
  lines.push(`### ▶ ${e.topic}  ·  T${e.tier}`);
  lines.push(`- **Primary (pillar):** "${e.primary.kw}" — ${e.primary.vol.toLocaleString()} vol / KD ${e.primary.kd}`);
  lines.push(`- **Quick-win entries:** ${e.candidates.length ? e.candidates.map((k) => `"${k.kw}" (${k.vol.toLocaleString()}/KD ${k.kd})`).join(" · ") : "— none under KD 30; this is a hard pillar"}`);
  lines.push(`- **Cluster (secondaries):** ${e.cluster.map((k) => `${k.kw} (KD ${k.kd})`).join(" · ")}`);
  lines.push(`- _topic: ${e.totalVol.toLocaleString()} vol · ${e.winnable} winnable(KD≤35) · medKD ${e.medKD}_`);
  return lines.join("\n");
};

L.push(`\n## ▶ Write next (${todo.length})`);
for (const t of [1, 2, 3]) {
  const g = todo.filter((e) => e.tier === t);
  if (!g.length) continue;
  L.push(`\n### TIER ${t} — ${t === 1 ? "quick wins (KD ≤18)" : t === 2 ? "winnable (KD 19–30)" : "high-value, harder (KD 31+)"}`);
...
console.log(`Wrote ${out} — ${todo.length} to write (T1:${todo.filter((e)=>e.tier===1).length} T2:${todo.filter((e)=>e.tier===2).length} T3:${todo.filter((e)=>e.tier===3).length}) · ${doneList.length} done`);
console.log(`Top of the queue: ${todo.slice(0, 5).map((e) => `${e.topic}(T${e.tier})`).join(" · ")}`);
